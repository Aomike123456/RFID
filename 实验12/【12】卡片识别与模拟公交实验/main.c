 ...
 * 实验说明：断电前提下向开发板插入1602液晶模块，然后用USB线插入到
 ...
 类型，有学生卡和老年卡两种（学生卡打折扣费，老年卡免费乘车），除
 此之外都是用户卡（正常收费），“Num”是卡片的实际卡号，我们在开发板
 上方5-15cm处放置卡片即可识别卡片类型并播报扣费提示，以此模拟公交
 场景，学习卡号识别、身份验证等功能实现。
******************************************************************/
...
#define ID1 319631138UL		//学生卡（可按需设定和修改）
#define ID2 319635063UL		//老年卡（可按需设定和修改）
...
void main(void)
{
  ...
  LCD1602_DIS();  				//显示字符效果
	...
	TTS(6);while(!BUSY);		//播报6段并等待完成（模拟公交模式）
	while(1)
	{
		SN=Read_Card();				//读取卡号数据
		if(SN)								//若卡号不为0，即有效卡号
		{
			...
			switch(SN)									//播报并显示卡片类型
			{
				...
				{
					//显示卡片类型为“Student”
					LCD1602_DIS_CHAR(1,5, 'S');
					LCD1602_DIS_CHAR(1,6, 't');
					LCD1602_DIS_CHAR(1,7, 'u');
					LCD1602_DIS_CHAR(1,8, 'd');
					LCD1602_DIS_CHAR(1,9, 'e');
					LCD1602_DIS_CHAR(1,10,'n');
					LCD1602_DIS_CHAR(1,11,'t');
					TTS(1);while(!BUSY);		//播报1段并等待完成（无内容）
					TTS(2);while(!BUSY);		//播报2段并等待完成（滴）
					TTS(22);while(!BUSY);		//播报22段并等待完成（学生卡）
					TTS(28);while(!BUSY);		//播报28段并等待完成（扣费成功）					
				}break;
				case ID2:
				{
					//显示卡片类型为“Old Man”
					LCD1602_DIS_CHAR(1,5, 'O');
					LCD1602_DIS_CHAR(1,6, 'l');
					LCD1602_DIS_CHAR(1,7, 'd');
					LCD1602_DIS_CHAR(1,8, ' ');
					LCD1602_DIS_CHAR(1,9, 'M');
					LCD1602_DIS_CHAR(1,10,'a');
					LCD1602_DIS_CHAR(1,11,'n');
					TTS(1);while(!BUSY);		//播报1段并等待完成（无内容）
					TTS(2);while(!BUSY);		//播报2段并等待完成（滴）
					TTS(23);while(!BUSY);		//播报23段并等待完成（老年卡）
					TTS(29);while(!BUSY);		//播报29段并等待完成（免费乘车）	
				}break;
				default:
				{
					//显示卡片类型为“GenUser”
					LCD1602_DIS_CHAR(1,5, 'G');
					LCD1602_DIS_CHAR(1,6, 'e');
					LCD1602_DIS_CHAR(1,7, 'n');
					LCD1602_DIS_CHAR(1,8, 'U');
					LCD1602_DIS_CHAR(1,9, 's');
					LCD1602_DIS_CHAR(1,10,'e');
					LCD1602_DIS_CHAR(1,11,'r');
					TTS(1);while(!BUSY);		//播报1段并等待完成（无内容）
					TTS(2);while(!BUSY);		//播报2段并等待完成（滴）
